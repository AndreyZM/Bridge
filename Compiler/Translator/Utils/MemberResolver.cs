...
namespace Bridge.Translator
{
    public class MemberResolver : IMemberResolver
    {
        ...
        private Dictionary<SyntaxTree, CSharpAstResolver> resolvers = new Dictionary<SyntaxTree, CSharpAstResolver>();
        ...
        private void InitResolver(SyntaxTree syntaxTree)
        {
            if (this.lastFileName != syntaxTree.FileName || string.IsNullOrEmpty(syntaxTree.FileName))
            {
                ...
                if (resolvers.TryGetValue(syntaxTree, out this.resolver))
                    return;

                var typeSystem = this.GetTypeSystem(syntaxTree);
                resolvers[syntaxTree] = this.resolver = new CSharpAstResolver(this.compilation, syntaxTree, typeSystem);
            }
        }
        ...
        public ResolveResult ResolveNode(AstNode node, ILog log)
        {
            lock (locker)
            {
                ...
                if (node.Parent != null && result is MethodGroupResolveResult)
                {
